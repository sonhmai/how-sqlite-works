...
mod tests {
    ...
    fn test_parse_leaf_table_cell_apples_sample_db() {
        ...
        // rowid 1
        let cell_bytes: Vec<u8> = vec![27, 1, 4, 0, 37, 35, 71, 114, 97, 110, 110, 121, 32, 83, 109, 105, 116, 104, 76, 105, 103, 104, 116, 32, 71, 114, 101, 101, 110];
        let cell = LeafTableCell::parse(cell_bytes.as_slice()).unwrap();
        assert_eq!(cell.rowid, 1);
        assert_eq!(cell.payload.values.len(), 3);
        assert_eq!(cell.payload.values[0], ColumnValue::Null);
        assert_eq!(cell.payload.values[1], ColumnValue::Text("Granny Smith".to_owned()));
        assert_eq!(cell.payload.values[2], ColumnValue::Text("Light Green".to_owned()));
    }
